...
class App extends React.Component {
  ...
  render() {
    const allOpen = this.state.tocos.every(t => t.open)
    return (
      <div>
        <button onClick={this.toggleAll}>Toggle All</button>
        <button onClick={this.toggleAll}>{this.state.allOpen ? "Close" : "Open"} All</button>
        ...
        <div>
          {this.state.tacos.map(taco => (
            <ContentToggle
              ...
              onToggle={isOpen => this.handleTacoToggle
              }
            >
              <div
                style={{
